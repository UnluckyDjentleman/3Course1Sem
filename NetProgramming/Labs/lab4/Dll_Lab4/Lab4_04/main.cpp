...
int main(int argc, char* argv[]) {
	...
	try {
		...
		while (true) {
			...
			HT::Element* elem = HT::Get(ht,new HT::Element(key.c_str(),key.length()+1));
			if (elem) {
				cout << "Element exists" << endl;
				string newPayload = to_string(stoi((char*)elem->payload) + 1);
				if (HT::Update(ht, elem, newPayload.c_str(), newPayload.length() + 1)) {
					cout << "Element Updated" << endl;
				}
				else {
					cout << "Cannot update Element" << endl;
				}
			}
			else {
				cout << "Retry to find" << endl;
			}
			delete elem;
			Sleep(1000);
		}
