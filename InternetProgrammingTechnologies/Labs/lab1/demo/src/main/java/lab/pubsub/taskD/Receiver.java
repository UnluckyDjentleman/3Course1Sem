...
public class Receiver implements MessageListener{
    ...
    Receiver() {
        try (JMSContext jmsContext = cf.createContext("admin", "admin")) {
            ...
            Destination messageTopic = jmsContext.createTopic("Ex_3d");
            String selector="symbol='BelSTU'";
            consumer = jmsContext.createConsumer(messageTopic,selector);
            consumer.setMessageListener(this);
            System.out.println("Listening to Ex_3d");
            while (true) {
                Thread.sleep(1000);
            }
        } catch (Exception e) {
